...
- 默认使用中文沟通、说明和评审。
- 执行任务时遵循目标目录下更具体的 `AGENTS.md`，并保持现有代码风格和目录约定。

## 动手前

- 先明确目标、范围、成功标准和不做什么；多步骤任务给出简短计划，并为每一步写明验证方式。
- 显式说明关键假设。存在歧义时列出不同理解及影响，不得静默选择；信息不足时先提问。
- 如果有明显更简单的方案，应主动提出；发现需求不合理或代价过高时说明理由和取舍。
- 修改项目文件，以及删除、移动、迁移、重构、安装依赖、执行 migration、修改 lockfile、`git add`、提交、推送或创建 PR 前，先给出可选方案及影响并等待确认；读取文件、只读检查和方案分析无需确认。
- 新发现的稳定约定，应先询问是否沉淀到公共规范，再修改 `AGENTS.md` 或相关架构文档。

## 简单且精准地修改

- 只实现请求范围内的最小改动，不增加未要求的功能、配置项、扩展点或单次使用的抽象。
- 不为现有不变量已排除的场景增加防御分支；实现明显可以更短、更直接时先简化。
- 不顺手重构、清理或格式化相邻代码。发现无关问题可以说明，但未经要求不处理。
- 每一处改动都应能直接追溯到用户请求；只清理由本次改动产生的无用 import、变量、函数和文件。
- 除非任务明确要求且已确认，不改变既有功能、API 语义、数据结构或用户可见行为。
...
- 错误日志应包含可追踪上下文，但不得泄露隐私，也不得输出密码、token、银行卡号、连接串等敏感信息。
