...
- 默认直接从具体模块导入，避免为了便利新增大范围 barrel 导出；体积较大的可选功能按交互时机懒加载。新增依赖、修改构建配置或引入跨路由拆包方案如未在当前请求中明确授权，须先确认；沿用现有方式的局部 `import()` 不需额外确认
...
- 移动端至少保证主要查询和录入流程可用
- 面向最终用户的删除、批量和不可逆操作，界面必须显示明确的二次确认
...
- 新增或实质改变交互、状态或业务行为的页面、store、service、hook 必须有对应测试；纯静态文案或样式调整按风险进行针对性验证
