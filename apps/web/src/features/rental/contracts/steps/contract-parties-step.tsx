...
export function ContractPartiesStep({
  ...
  const query = useQuery({
    ...rentalQueryOptions.tenants(api, organizationId, {
      keyword,
      page,
      pageSize: 20,
    }),
      ...
      if (!values.parties.some((party) => party.tenantId === tenant.id))
        onChange({
          ...
          parties: [
            ...
            {
              tenantId: tenant.id,
              isPrimaryPayer: values.parties.length === 0,
            },
          ],
        })
      setNewName('')
    ...
    <section aria-labelledby="contract-parties-title" className="space-y-4">
      ...
      <fieldset className="grid gap-2">
        ...
        <RadioGroup
          ...
          name="primary-payer"
          value={values.parties.find((party) => party.isPrimaryPayer)?.tenantId}
          onValueChange={(tenantId) =>
            onChange({
              ...values,
              parties: values.parties.map((party) => ({
                ...party,
                isPrimaryPayer: party.tenantId === tenantId,
              })),
            })
          }
        >
