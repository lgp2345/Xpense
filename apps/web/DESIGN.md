...
## 19. design-taste-frontend 技能应用规则

### 19.1 适用范围与优先级

设计和实现 Xpense Web 页面、组件、图表、交互状态或视觉重构时，必须使用 `$design-taste-frontend` 进行反模板化和交付前检查，但必须先判断其适用边界。

- `design-taste-frontend` 主要面向落地页、作品集和视觉重构，并明确不直接覆盖仪表盘、数据表格和高密度产品 UI。
- Xpense Web 是个人财务后台，本文档、`apps/web/AGENTS.md` 和 `apps/web/ARCHITECTURE.md` 的产品约束优先于技能中的营销页面默认值。
- 技能中的营销 hero、品牌 Logo 墙、每页必须使用真实图片、落地页内容密度和滚动叙事规则不适用于普通后台页面。
- 当前组件策略继续保持 HeroUI 为主、shadcn/ui 为补充。不得用技能的“单一设计系统”通用规则擅自更换现有组件库或安装依赖。
- 当前默认视觉语言是本文档定义的冷雾青白浅色界面。技能中的双主题默认要求不自动触发；新增深色模式或主题切换必须先形成方案并确认。
- 技能中的色彩限制应按财务语义适配。收入、支出、预算、警告和错误可以使用不同语义色，但同一语义在全站必须保持一致，不能把语义色当装饰色。

### 19.2 项目设计判断与参数

新页面或较大视觉调整在给出实现方案前，应先声明一行设计判断：

> Reading this as: 面向个人财务管理用户的高频后台工作台，采用冷雾青白、低对比但高可读的连续分析画布，基于 HeroUI、Tailwind CSS 和局部 CSS Modules。

默认参数为：

- `DESIGN_VARIANCE: 5`：允许不对称分析布局，但不牺牲扫描效率和业务秩序。
- `MOTION_INTENSITY: 3`：以 hover、press、状态过渡和一次性进入动效为主，不进行滚动劫持或展示性编舞。
- `VISUAL_DENSITY: 7`：保持较高信息密度，通过字号、留白、对齐和连续画布避免拥挤。

用户明确提出新的设计方向时可以调整参数，但必须解释调整原因，并继续遵守可访问性、业务语义和响应式约束。

### 19.3 保留并执行的技能规则

- 先理解页面类型、用户任务、现有品牌 Token 和安静约束，再决定布局和视觉处理，不套用 AI 紫蓝渐变、居中 hero、三张等宽卡片等默认模板
- 颜色、圆角、图标和材质必须在页面内保持一致，具体数值以本文档 Token 为准
- 卡片只用于真实层级、聚焦或独立操作，其他内容优先依靠连续画布、留白、对齐和稀疏分隔组织
- 每个数据模块覆盖加载、空、错误、部分数据和禁用状态；表单还必须覆盖校验错误、帮助文本和清晰焦点
- 动画只能表达层级、反馈、状态变化或数据关系，优先动画 `transform` 和 `opacity`，并尊重 `prefers-reduced-motion`
- 不使用 React state 跟踪指针位置、滚动进度或动画帧；滚动相关效果优先使用 Motion、IntersectionObserver 或 CSS 能力，不直接注册逐帧 React 更新
- 图标使用项目允许的统一图标家族，不手绘功能 SVG，不用 Emoji 替代功能图标
- 可见文案保持直接、准确和同一语气，不使用营销空话、假精确数据、模板人名、装饰性版本号或无业务意义的状态点
- 用户可见文案不使用 em dash 或 en dash 作为分隔符，中文内容使用合适的中文标点，范围和复合词使用普通连字符
- 全屏区域确有必要时使用动态视口高度，避免 `h-screen` 导致移动端视口跳动；普通后台页面不为了视觉效果强制占满首屏
- 引入 Motion、GSAP、Three.js、字体、图片服务或其他第三方资源前必须检查依赖并按项目规则确认，不得因为技能示例而自动安装

### 19.4 设计交付前检查

除第 18 节检查清单外，较大页面或视觉调整还必须确认：

- [ ] 是否声明了项目设计判断和三个设计参数？
- [ ] 是否明确区分后台产品 UI 与营销页规则，没有套用不适用的 hero、Logo 墙或图片要求？
- [ ] 是否保持冷雾青白主题、语义色映射和固定圆角 Token 一致？
- [ ] 按钮、表单、辅助文字、图表和透明表面是否满足 WCAG AA 对比要求？
- [ ] 是否没有重复意图的主操作，桌面按钮文字不会换行？
- [ ] 每个多列区域是否明确提供 `< 768px` 的单列或移动端替代布局？
- [ ] 加载、空、错误、部分数据、禁用和减少动效状态是否完整？
- [ ] 是否逐项检查所有可见文案，没有假数据、含糊指代或模板化营销表达？
- [ ] 每个动画是否能用层级、反馈、状态变化或数据关系解释，并且存在清理与降级？
- [ ] 是否没有手绘功能图标、装饰性假截图、无意义渐变和为降低对比而损害可读性的处理？
