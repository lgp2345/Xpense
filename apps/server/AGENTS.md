...
后端服务是系统的业务边界和数据访问入口。
所有账户、分类、交易、预算、统计、用户配置等业务规则应优先在服务端落地，客户端只负责展示、交互和必要的轻量校验。
...
- `src/config`：配置加载和校验
- `src/db`：Drizzle 连接、schema、migration 入口
- `src/logger`：结构化日志适配
...
- NestJS 应通过 `@nestjs/platform-fastify` 使用 Fastify adapter
- 启动逻辑必须显式使用 `NestFastifyApplication`
- E2E 测试应使用 Fastify 的 `app.inject`
- Fastify plugin 必须集中注册，避免分散在业务模块中
- 不直接依赖 Express 专属 middleware 或 request/response API

## Drizzle 规则

- 数据库 schema、连接和 migration 统一放在 `src/db`
- 所有数据库访问必须通过 repository 或明确的数据访问层
- 禁止在 controller 中直接调用 Drizzle
- 查询应保持类型安全，避免拼接原始 SQL 字符串
