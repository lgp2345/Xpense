...
describe("IamService", () => {
  ...
  it.each([
    ...
    const legacyPayload: UpdateRoleDto = {
      name: "Ledger Owner",
      permissionKeys,
    };

    await expect(
      service.updateRole(authContext, "role-custom", legacyPayload),
    ).rejects.toBeInstanceOf(ForbiddenException);
  ...
  it("does not inspect permission ceilings during valid role profile updates", async () => {
    ...
    await expect(
      service.updateRole(limitedContext, "role-custom", { name: "Ledger Owner" }),
    ).resolves.toEqual(editableRole);
  ...
  it("updates editable role details without writing permission changes or permission audits", async () => {
    ...
    await service.updateRole(authContext, "role-custom", { name: "Ledger Owner" });
